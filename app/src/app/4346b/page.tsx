
export default function Page() {
    return (
        <div className="raw p-10">
            <h2 className="text-[20px] lb:text-[24px] text-center mt-10 text-gray-400 font-bold">
                Parabéns, você é realmente acima da média.
            </h2>

            <h3 className="text-[20px] lb:text-[24px] text-center mt-10 text-gray-400 font-bold">
                Aqui está a sua seedphrase, seja rápido:
            </h3>

            <p className="text-[16px] lb:text-[24px] text-center mt-10 text-gray-400 font-bold">
                1999 1759 694 1592 1649 463 185 759 221 774 686 855
            </p>

            <p className="text-[16px] lb:text-[24px] text-center mt-10 text-gray-400 font-bold">
                https://gist.github.com/DMeechan/9761fea1031c95f71c39ac2b80884bc5
            </p>
        </div>
    )
}
